...
class Analyzer:
    ...
    def __init__(self, cache_queue: multiprocessing.Queue, writing_frquency_time=60, writing_frequency_number=0, file_name_frequency_time="CHR_by_time", file_name_frequency_number="CHR_regular"):
        """
        ...
        if self.__frequency_number!=0:
            self.__file = open(file_name_frequency_number+".csv", "w", encoding='UTF8') # Open txt file for writing the analyzes results
        ...
        if self.__frequency_time!=0:
            self.__file_time = open(file_name_frequency_time+".csv", "w", encoding='UTF8') # Open txt file for writing the analyzes results
    ...
    def receive_status(self):
        ...
        while status is not None: # None is sent by the cache simulation when the simulation is over
            ...
            if (self.__hit+self.__miss+self.__pass)-self.__last_total>=self.__frequency_number and self.__frequency_number!=0:
            ...
            if status[0]-self.__last_time>=self.__frequency_time and self.__frequency_time!=0:
    ...
    def save_frequency_results(self):
        ...
        """
        self.__writer.writerow([self.__hit+self.__miss+self.__pass, self.__hit, self.__miss, self.__pass, round(self.cache_hit_ratio()*100,3)]) # cache hit ratio (CHR) writing
        self.__file.flush()
    ...
    def save_time_results(self):
        ...
        self.__writer_time.writerow([DT.datetime.utcfromtimestamp(self.__last_time).isoformat(), hit+miss+pass_, hit, miss, pass_, round((hit/(hit+miss+pass_))*100,3)]) # cache hit ratio (CHR) writing
